...
class LasryDataModule(PerturbationDataModule):
    def __init__(
            self,
            batch_size: int = 128,
        ...
        # Metadata processing
        self.adata.obs['is_supervised'] = True
        self.adata.obs['domain'] = self.adata.obs['donor_id'].astype(str)
        self.adata.obs.loc[self.adata.obs['malignant'] == 'Control', 'domain'] = 'healthy'

        self.adata_train = self.adata[~self.adata.obs['is_ood']]
        self.adata_train.obs['index'] = range(self.adata_train.shape[0])

        # define splits
        train_idx = self.adata_train[~self.adata_train.obs["is_test"]].obs["index"]
        test_idx = self.adata_train[self.adata_train.obs["is_test"]].obs["index"]
        train_idx, val_idx = train_test_split(train_idx, train_size=0.9, random_state=0)
